...
const BlogOther: NextPage<PageData> = ({ dataProps }) => {
    ...
    return (
        <div className="bg-pageBg/90 overflow-x-hidden">
            ...
            <BlogContentWrapper dataProps={dataProps} fetchMoreQueryString="Other" />
        </div>
    );
};
...
export async function getStaticProps() {
    const {
        ...
        query: gql`
            query DevBlog {
                devBlogs(filters: { tag: { eq: "Other" } }, sort: "date:desc") {
                    data {
                        attributes {
                            mainPageHeaderTitle
                            title

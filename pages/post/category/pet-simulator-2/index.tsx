...
const BlogPetSimulator2: NextPage<PageData> = ({ dataProps }) => {
    ...
    return (
        <div className="bg-pageBg/90 overflow-x-hidden">
            ...
            <BlogContentWrapper dataProps={dataProps} fetchMoreQueryString="Pet Simulator 2" />
        </div>
    );
};

export default BlogPetSimulator2;
...
export async function getStaticProps() {
    const {
        ...
        query: gql`
            query DevBlog {
                devBlogs(filters: { tag: { eq: "Pet Simulator 2" } }, sort: "date:desc") {
                    data {
                        attributes {
                            mainPageHeaderTitle
                            title

...
const BlogBigPaintball: NextPage<PageData> = ({ dataProps }) => {
    ...
    return (
        <div className="bg-pageBg/90 overflow-x-hidden">
            ...
            <BlogContentWrapper dataProps={dataProps} fetchMoreQueryString="BIG Paintball" />
        </div>
    );
};

export default BlogBigPaintball;
...
export async function getStaticProps() {
    const {
        ...
        query: gql`
            query DevBlog {
                devBlogs(filters: { tag: { eq: "BIG Paintball" } }, sort: "date:desc") {
                    data {
                        attributes {
                            mainPageHeaderTitle
                            title
